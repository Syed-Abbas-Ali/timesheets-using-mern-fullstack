...
const Timesheet= () => {
  ...
  let username=gettinguser.email;
  let empid=gettinguser.employeeid;
...
const store=(e)=>{
  ...
  const tsheetsubmit = async (e) => {
    e.preventDefault();
    await axios.post("https://inkprog-timesheets.herokuapp.com/api/signinup/timesheet",updatesheet)
    ...
    <div className="timesheets">
      <div className="ts1">
        <div className="details">
          <h3>Employee Name : <span>{username}</span></h3>
          <h3>Employee ID : <span>{empid}</span></h3>
        </div>
